    ...
    <Card title={t('invoices.detail.companyInfoTitle')} action={action}>
      {isEditing && form ? (
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          {senderName && (
            <div className="rounded-xl border border-slate-300 p-4 shadow-sm dark:border-slate-700">
              <div className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
                <div className="flex items-start gap-3 border-b border-slate-100 pb-4 dark:border-slate-700">
                  <Building2 size={18} className="mt-0.5 shrink-0 text-blue-900 dark:text-blue-400" />
                ...
                {(senderAddress || senderLocationLine) && (
                  <div className="flex items-start gap-3 border-b border-slate-100 pb-4 dark:border-slate-700">
                    <MapPin size={18} className="mt-0.5 shrink-0 text-blue-900 dark:text-blue-400" />
                    <div className="flex flex-col gap-1">
                      <span className="text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-500">
                        {t('invoices.detail.recipientAddressLabel')}
                      </span>
                      {senderAddress && <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{senderAddress}</span>}
                      {senderLocationLine && <span className="text-sm text-slate-700 dark:text-slate-300">{senderLocationLine}</span>}
                    </div>
                  </div>
                )}
                ...
                {(senderTaxOffice || senderTaxNumber) && (
                  <div className="flex items-start gap-3">
                    <Receipt size={18} className="mt-0.5 shrink-0 text-blue-900 dark:text-blue-400" />
                    <div className="flex flex-col gap-2">
                      ...
                      {senderTaxOffice && (
                        <span className="text-sm text-slate-500 dark:text-slate-400">
                          {t('customers.form.taxOffice')}: <span className="font-bold text-slate-900 dark:text-slate-100">{senderTaxOffice}</span>
                        </span>
                      )}
                      {senderTaxNumber && (
                        <span className="text-sm text-slate-500 dark:text-slate-400">
                          {t('customers.form.taxNumber')}: <span className="font-bold text-slate-900 dark:text-slate-100">{senderTaxNumber}</span>
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {(senderEmail || senderPhone) && (
                  <div className="flex items-start gap-3">
                    <div className="mt-0.5 flex shrink-0 items-center gap-0.5 text-blue-900 dark:text-blue-400">
                      <Mail size={16} />
                      <Phone size={16} />
                    </div>
                    <div className="flex flex-col gap-2">
                      ...
                      {senderEmail && (
                        <span className="text-sm text-slate-500 dark:text-slate-400">
                          {t('customers.form.email')}: <span className="font-bold text-slate-900 dark:text-slate-100">{senderEmail}</span>
                        </span>
                      )}
                      {senderPhone && (
                        <span className="text-sm text-slate-500 dark:text-slate-400">
                          {t('customers.form.phone')}: <span className="font-bold text-slate-900 dark:text-slate-100">{senderPhone}</span>
                        </span>
                      )}
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
              <div className="flex items-start gap-3 border-b border-slate-100 pb-4 dark:border-slate-700">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{recipientName}</span>
                </div>
              </div>
              ...
              {(address || locationLine) && (
                <div className="flex items-start gap-3 border-b border-slate-100 pb-4 dark:border-slate-700">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {address && <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{address}</span>}
                    {locationLine && <span className="text-sm text-slate-700 dark:text-slate-300">{locationLine}</span>}
                  </div>
                </div>
              )}
              ...
              {(taxOffice || taxNumber) && (
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {taxOffice && (
                      <span className="text-sm text-slate-500 dark:text-slate-400">
                        {t('customers.form.taxOffice')}: <span className="font-bold text-slate-900 dark:text-slate-100">{taxOffice}</span>
                      </span>
                    )}
                    {taxNumber && (
                      <span className="text-sm text-slate-500 dark:text-slate-400">
                        {t('customers.form.taxNumber')}: <span className="font-bold text-slate-900 dark:text-slate-100">{taxNumber}</span>
                      </span>
                    )}
                  </div>
                </div>
              ...
              {(email || phone) && (
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {email && (
                      <span className="text-sm text-slate-500 dark:text-slate-400">
                        {t('customers.form.email')}: <span className="font-bold text-slate-900 dark:text-slate-100">{email}</span>
                      </span>
                    )}
                    {phone && (
                      <span className="text-sm text-slate-500 dark:text-slate-400">
                        {t('customers.form.phone')}: <span className="font-bold text-slate-900 dark:text-slate-100">{phone}</span>
                      </span>
                    )}
                  </div>
                </div>
