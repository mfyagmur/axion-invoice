    ...
    <div className="flex gap-1">
      {ALIGN_OPTIONS.map(({ value: v, icon: Icon }) => (
        <button
          ...
          onClick={() => onChange(v)}
          className={twMerge(
            'flex-1 rounded-md border border-slate-300 py-1.5 flex items-center justify-center hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-800',
            value === v && 'border-slate-900 bg-slate-900 text-white hover:bg-slate-900 dark:border-slate-100 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-100',
          )}
        >
          <Icon size={14} />
        </button>
...
export function PropertiesPanel() {
  ...
  if (selectedIds.length === 0) {
    return <div className="w-full text-sm text-slate-400 dark:text-slate-500 lg:w-72 lg:shrink-0">{t('editor.properties.emptyState')}</div>
  }

  if (selectedIds.length > 1) {
    return (
      <div className="flex w-full flex-col gap-3 lg:w-72 lg:shrink-0">
        <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{t('editor.properties.multiSelected', { count: selectedIds.length })}</h2>
        <div className="flex gap-2">
          <button type="button" onClick={() => duplicateElements(selectedIds)} className="flex items-center gap-1 rounded-md border border-slate-300 px-2 py-1.5 text-xs hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-800">
            <Copy size={14} /> {t('editor.properties.duplicate')}
          </button>
          <button type="button" onClick={() => removeElements(selectedIds)} className="flex items-center gap-1 rounded-md border border-red-300 px-2 py-1.5 text-xs text-red-600 hover:bg-red-50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-950/40">
  ...
  return (
    <div className="flex w-full flex-col gap-4 lg:w-72 lg:shrink-0">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{t(`editor.elementType.${element.type}`)}</h2>
        <div className="flex gap-1">
          <button type="button" onClick={() => toggleLock(element.id)} className="rounded p-1 text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800" title={t('editor.properties.lock')}>
            {element.locked ? <Lock size={14} /> : <Unlock size={14} />}
          </button>
          <button type="button" onClick={() => toggleHidden(element.id)} className="rounded p-1 text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800" title={t('editor.properties.hide')}>
            {element.hidden ? <EyeOff size={14} /> : <Eye size={14} />}
          </button>
          <button type="button" onClick={() => duplicateElements([element.id])} className="rounded p-1 text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800" title={t('editor.properties.duplicate')}>
            <Copy size={14} />
          </button>
          <button type="button" onClick={() => removeElements([element.id])} className="rounded p-1 text-red-500 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/40" title={t('editor.properties.delete')}>
            <Trash2 size={14} />
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <button type="button" onClick={() => bringToFront(element.id)} className="flex flex-1 items-center justify-center gap-1 rounded-md border border-slate-300 py-1.5 text-xs hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-800">
          <BringToFront size={14} /> {t('editor.layers.bringToFront')}
        </button>
        <button type="button" onClick={() => sendToBack(element.id)} className="flex flex-1 items-center justify-center gap-1 rounded-md border border-slate-300 py-1.5 text-xs hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-800">
        ...
        <>
          {element.type === 'text' && (
            <Field label={t('editor.properties.content')}>
              <textarea
                ...
                rows={3}
                className="rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-900 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
              />
            </Field>
          )}
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Field label={t('editor.properties.color')}>
              <input type="color" value={element.color} onChange={(e) => set({ color: e.target.value })} className="h-9 w-full rounded-md border border-slate-300 dark:border-slate-600" />
            </Field>
          </div>
          <div className="flex gap-2">
            <button type="button" onClick={() => set({ font_weight: element.font_weight === 'bold' ? 'normal' : 'bold' })} className={twMerge('flex-1 rounded-md border border-slate-300 py-1.5 flex items-center justify-center dark:border-slate-600', element.font_weight === 'bold' && 'border-slate-900 bg-slate-900 text-white dark:border-slate-100 dark:bg-slate-100 dark:text-slate-900')}>
              <Bold size={14} />
            </button>
            <button type="button" onClick={() => set({ font_style: element.font_style === 'italic' ? 'normal' : 'italic' })} className={twMerge('flex-1 rounded-md border border-slate-300 py-1.5 flex items-center justify-center dark:border-slate-600', element.font_style === 'italic' && 'border-slate-900 bg-slate-900 text-white dark:border-slate-100 dark:bg-slate-100 dark:text-slate-900')}>
              <Italic size={14} />
            </button>
          </div>
      ...
      {element.type === 'line' && (
        <div className="grid grid-cols-2 gap-2">
          <Field label={t('editor.properties.color')}>
            <input type="color" value={element.stroke_color} onChange={(e) => set({ stroke_color: e.target.value })} className="h-9 w-full rounded-md border border-slate-300 dark:border-slate-600" />
          </Field>
      ...
      {element.type === 'rectangle' && (
        <div className="grid grid-cols-2 gap-2">
          <Field label={t('editor.properties.fillColor')}>
            <input type="color" value={element.fill_color ?? '#ffffff'} onChange={(e) => set({ fill_color: e.target.value })} className="h-9 w-full rounded-md border border-slate-300 dark:border-slate-600" />
          </Field>
          <Field label={t('editor.properties.borderColor')}>
            <input type="color" value={element.border_color} onChange={(e) => set({ border_color: e.target.value })} className="h-9 w-full rounded-md border border-slate-300 dark:border-slate-600" />
          </Field>
      ...
      {(element.type === 'logo' || element.type === 'image') && (
        <Field label={t('editor.properties.objectFit')}>
          <select value={element.object_fit} onChange={(e) => set({ object_fit: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
      ...
      {element.type === 'qrcode' && (
        <>
          <Field label={t('editor.properties.qrDataSource')}>
            <select value={element.data_source} onChange={(e) => set({ data_source: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
      ...
      {element.type === 'bank-account' && (
        <>
          <Field label={t('editor.properties.bankAccountSlot')}>
            <select value={element.slot} onChange={(e) => set({ slot: Number(e.target.value) })} className="rounded-md border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Field label={t('editor.table.headerBg')}>
              <input type="color" value={element.header_bg_color} onChange={(e) => set({ header_bg_color: e.target.value })} className="h-9 w-full rounded-md border border-slate-300 dark:border-slate-600" />
            </Field>
